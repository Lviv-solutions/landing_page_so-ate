...
// ----------------------------------------------------------------------
export function PhoneInput({ sx, size, label, placeholder, fullWidth = true, variant: variantProp, 
/********/
value, country, onChange, defaultCountry, 
/********/
hideSelect, ...other }) {
    ...
    const [selectedCountry, setSelectedCountry] = useState(parseCountryFromPhone(normalizedValue) ?? country ?? defaultCountry);
    ...
    const debouncedChange = useMemo(() => debounce((inputValue) => onChange(inputValue), 200), [onChange]);
    ...
    const handleSelectedCountry = useCallback((countryCode) => {
        setSearchCountry('');
        handleClearInput();
        setSelectedCountry(countryCode);
    }, [handleClearInput]);
    const renderSelect = () => (_jsx(CountryListPopover, { options: countries, searchCountry: searchCountry, selectedCountry: activeCountry, onSearchCountry: handleSearchCountry, onSelectedCountry: handleSelectedCountry, disabled: isCountryLocked, sx: {
    ...
    const renderInput = () => {
        const textFieldProps = {
            ...
            slotProps: {
                ...
                input: {
                    endAdornment: normalizedValue && (_jsx(InputAdornment, { position: "end", children: _jsx(IconButton, { size: "small", edge: "end", onClick: handleClearInput, children: _jsx(Iconify, { width: 16, icon: "mingcute:close-line" }) }) })),
                },
            },
        };
        ...
        return _jsx(PhoneNumberInput, { ...textFieldProps, ...phoneInputProps, ...other });
    };
    ...
    return (_jsxs(Box, { sx: [
        ...
        ], children: [!hideSelect && renderSelect(), renderInput()] }));
}
// ----------------------------------------------------------------------
function CustomInput({ ref, ...other }) {
    return _jsx(TextField, { inputRef: ref, ...other });
}
// ----------------------------------------------------------------------
