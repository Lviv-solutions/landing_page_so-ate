...
export function RHFUploadAvatar({ name, slotProps, ...other }) {
    ...
    return (_jsx(Controller, { name: name, control: control, render: ({ field, fieldState: { error } }) => {
            ...
            return (_jsxs(Box, { ...slotProps?.wrapper, children: [_jsx(UploadAvatar, { value: field.value, error: !!error, onDrop: onDrop, ...other }), _jsx(HelperText, { errorMessage: error?.message, sx: { justifyContent: 'center' } })] }));
        } }));
}
// ----------------------------------------------------------------------
