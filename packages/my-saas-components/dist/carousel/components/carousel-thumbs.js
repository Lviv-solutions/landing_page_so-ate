...
// ----------------------------------------------------------------------
export function CarouselThumbs({ sx, options, children, slotProps, className, ...other }) {
    const axis = options?.axis ?? 'x';
    const slideSpacing = options?.slideSpacing ?? '12px';
    const renderChildren = () => Children.map(children, (child) => {
        if (isValidElement(child)) {
            const reactChild = child;
            return (_jsx(CarouselSlide, { options: { ...options, slideSpacing }, sx: slotProps?.slide, children: child }, reactChild.key));
        }
        return null;
    });
    return (_jsx(ThumbsRoot, { axis: axis, enableMask: !slotProps?.disableMask, className: mergeClasses([carouselClasses.thumbs.root, className]), sx: sx, ...other, children: _jsx(ThumbsContainer, { axis: axis, slideSpacing: slideSpacing, className: carouselClasses.thumbs.container, sx: slotProps?.container, children: renderChildren() }) }));
}
