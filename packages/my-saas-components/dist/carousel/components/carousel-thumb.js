...
export function CarouselThumb({ sx, src, index, selected, className, ...other }) {
    return (_jsx(ThumbRoot, { selected: selected, className: mergeClasses([carouselClasses.thumbs.item, className]), sx: sx, ...other, children: _jsx("img", { alt: `carousel-thumb-${index}`, src: src, className: carouselClasses.thumbs.image }) }));
}
// ----------------------------------------------------------------------
