...
export function CarouselDotButtons({ sx, gap, slotProps, className, onClickDot, scrollSnaps, selectedIndex, variant = 'circular', ...other }) {
    ...
    return (_jsx(Box, { component: "ul", className: mergeClasses([carouselClasses.dots.root, className]), sx: [
            {
                zIndex: 9,
                display: 'flex',
                gap: `${dotGap}px`,
        ...
        ], ...other, children: scrollSnaps.map((_, index) => {
            const isSelected = index === selectedIndex;
            return (_jsx("li", { children: _jsx(DotItem, { disableRipple: true, "aria-label": `dot-${index}`, size: dotSize, variant: variant, selected: isSelected, className: mergeClasses(carouselClasses.dots.item, {
                        [carouselClasses.dots.itemSelected]: isSelected,
                    }), onClick: () => onClickDot(index), sx: slotProps?.dot?.sx, children: variant === 'number' && index + 1 }) }, index));
        }) }));
}
