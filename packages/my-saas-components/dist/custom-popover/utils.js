...
function flipHorizontal(origin) {
    if (origin.horizontal === 'left')
        return { ...origin, horizontal: 'right' };
    if (origin.horizontal === 'right')
        return { ...origin, horizontal: 'left' };
    return origin;
}
/**
 * Gets the popover origin pair for a given placement, with optional RTL flipping.
 */
export function getPopoverOrigin(placement, isRtl = false) {
 ...
 */
export function getArrowOffset(anchorRect, paperRect, arrowSize) {
