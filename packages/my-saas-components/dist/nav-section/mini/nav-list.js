...
export function NavList({ data, depth, render, cssVars, slotProps, checkPermissions, enabledRootRedirect, }) {
    ...
    const renderDropdown = () => !!data.children && (_jsx(NavDropdown, { disableScrollLock: true, "aria-hidden": !open, id: id, open: open, anchorEl: anchorEl, anchorOrigin: { vertical: 'center', horizontal: isRtl ? 'left' : 'right' }, transformOrigin: { vertical: 'center', horizontal: isRtl ? 'right' : 'left' }, slotProps: {
            paper: {
                onMouseEnter: handleOpenMenu,
                onMouseLeave: onClose,
                className: navSectionClasses.dropdown.root,
            },
        }, sx: { ...cssVars }, children: _jsx(NavDropdownPaper, { className: navSectionClasses.dropdown.paper, sx: slotProps?.dropdown?.paper, children: _jsx(NavSubList, { data: data.children, depth: depth, render: render, cssVars: cssVars, slotProps: slotProps, checkPermissions: checkPermissions, enabledRootRedirect: enabledRootRedirect }) }) }));
    // Hidden item by role
