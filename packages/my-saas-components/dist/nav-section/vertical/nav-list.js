...
export function NavList({ data, depth, render, slotProps, checkPermissions, enabledRootRedirect, }) {
    ...
    const renderNavItem = () => (_jsx(NavItem, { ref: navItemRef, 
        ...
        // state
        open: open, active: isActive, disabled: data.disabled, 
        // options
    ...
    const renderCollapse = () => !!data.children && (_jsx(NavCollapse, { mountOnEnter: true, unmountOnExit: true, depth: depth, in: open, "data-group": data.title, children: _jsx(NavSubList, { data: data.children, render: render, depth: depth, slotProps: slotProps, checkPermissions: checkPermissions, enabledRootRedirect: enabledRootRedirect }) }));
    // Hidden item by role
    ...
    return (_jsxs(NavLi, { disabled: data.disabled, sx: {
        ...
        }, children: [renderNavItem(), renderCollapse()] }));
}
// ----------------------------------------------------------------------
function NavSubList({ data, render, depth = 0, slotProps, checkPermissions, enabledRootRedirect, }) {
    return (_jsx(NavUl, { sx: { gap: 'var(--nav-item-gap)' }, children: data.map((list) => (_jsx(NavList, { data: list, render: render, depth: depth + 1, slotProps: slotProps, checkPermissions: checkPermissions, enabledRootRedirect: enabledRootRedirect }, list.title))) }));
}
