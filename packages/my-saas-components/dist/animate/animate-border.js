...
export function AnimateBorder({ sx, children, duration, slotProps, className, ...other }) {
    ...
    const outlineColor = typeof slotProps?.outlineColor === 'function'
    ...
    const renderPrimaryBorder = () => (_jsx(MovingBorder, { ...borderProps, ref: primaryBorderRef, size: slotProps?.primaryBorder?.size, sx: [
            {
    ...
    const renderSecondaryBorder = () => slotProps?.secondaryBorder && (_jsx(MovingBorder, { ...borderProps, size: slotProps?.secondaryBorder?.size ?? slotProps?.primaryBorder?.size, sx: [
            {
    ...
    return (_jsxs(Box, { dir: "ltr", ref: rootRef, className: mergeClasses([animateBorderClasses.root, className]), sx: [
            {
                minWidth: 40,
                minHeight: 40,
                overflow: 'hidden',
        ...
        ], ...other, children: [renderPrimaryBorder(), renderSecondaryBorder(), children] }));
}
function MovingBorder({ sx, size, isHidden, rx = '30%', ry = '30%', duration = 8, ...other }) {
    ...
    return (_jsxs(Box, { component: "span", sx: [{ textAlign: 'initial' }, ...(Array.isArray(sx) ? sx : [sx])], ...other, children: [_jsx("svg", { xmlns: "http://www.w3.org/2000/svg", preserveAspectRatio: "none", width: "100%", height: "100%", className: animateBorderClasses.svgWrapper, style: { position: 'absolute' }, children: _jsx("rect", { ref: svgRectRef, fill: "none", width: "100%", height: "100%", rx: rx, ry: ry }) }), _jsx(Box, { component: m.span, style: { transform }, className: animateBorderClasses.movingShape, sx: {
                    width: size,
                    height: size,
                    filter: 'blur(8px)',
                    position: 'absolute',
