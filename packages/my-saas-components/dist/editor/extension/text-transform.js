...
const isValidTextTransform = (value, allowed) => typeof value === 'string' && allowed.includes(value);
// ----------------------------------------------------------------------
export const TextTransform = Mark.create({
            ...
            textTransform: {
                ...
                parseHTML: (element) => {
                    if (!isHTMLElement(element))
                        return undefined;
                    const rawValue = element.style.textTransform ||
    ...
    addCommands() {
        return {
             ...
             */
            setTextTransform: (value) => ({ commands }) => {
                if (!isValidTextTransform(value, this.options.allowedValues))
                    return false;
             ...
             */
            unsetTextTransform: () => ({ commands }) => commands.unsetMark(this.name),
            /**
             * ➤ Toggle the text transform
             */
            toggleTextTransform: (value) => ({ editor, commands }) => {
                if (!isValidTextTransform(value, this.options.allowedValues))
                    return false;
